...
CRGBPalette16 springPalette;
CRGBPalette16 amherstPalette;
...
void FillLEDsFromPaletteColors( uint8_t colorIndex)
{
  ...
  for ( int i = 0; i < NUM_LEDS; i++) {
    leds[i] = ColorFromPalette( amherstPalette, colorIndex, brightness, currentBlending);
    leds2[i] = leds[i];
    leds3[i] = leds[i];
    leds4[i] = leds[i];
    leds5[i] = leds[i];
...
void SetLightPulse(int pulseStartPos, int pulseWidth) {
  ...
  for (int dot = pulseStartPos; dot < pulseStartPos + pulseWidth; dot++) {
    leds[dot] = CRGB(0, 50, 255); //green pulse
    leds2[dot] = leds[dot];
    leds3[dot] = leds[dot];
    leds4[dot] = leds[dot];
    leds5[dot] = leds[dot];
...
//SpringColors:
  CRGB springorange = CRGB(255, 0, 60); //orange
  CRGB darkpink = CRGB(255,150,0);
  CRGB purple = CRGB(100,250,50);
  CRGB lightpink = CRGB(250,150,75);
  CRGB black = CRGB(0,0,0);

//AmherstWhite //RBG color order
  CRGB apurple = CRGB(200,240,50);
  CRGB awhite = CRGB(255, 120, 150);
  ...
  springPalette = CRGBPalette16(black, springorange, springorange, purple, purple, black, darkpink, lightpink, springorange, springorange, black, darkpink, darkpink, lightpink, purple, purple);
  amherstPalette = CRGBPalette16(awhite, apurple, apurple, awhite, apurple, awhite, apurple, awhite, apurple, apurple, awhite, apurple, apurple, awhite, apurple, apurple);
